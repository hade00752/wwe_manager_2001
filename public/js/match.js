// public/js/match.js
import {bootOrNull, headshotImg, saveState } from './engine.js';
...
export function openMatchModal(stateIn, matchId){
  const state = stateIn || bootOrNull();
  if (!state) {
    showModal(el('div',{}, el('div',{text:'No save found.'})));
    return;
  }
  ...
  add('Repeat penalty', (rec.penalties?.repeat ? `-${rec.penalties.repeat}` : (det.repeatPenalty?`-${det.repeatPenalty}`:'—')));
  ...
  const injuries = rec.injuries || rec.details?.injuries || [];

  if (injuries.length) {
    injuries.forEach(i => {
      const victim  = i.victim || i.name || i.wrestler || 'Unknown';
      const weeks   = (i.weeks ?? i.injuryWeeks ?? i.outWeeks ?? 0) | 0;
      const label   = i.injury || i.diagnosis || 'Injury';
      const injurer = i.injurer ? ` (caused by ${i.injurer})` : '';
      const sev     = (i.severity != null) ? ` — severity ${i.severity}` : '';
      injList.appendChild(
        el('li', { text: `${victim}${injurer}: ${label} — out ${weeks} week(s)${sev}` })
      );
    });
  } else {
  ...
  // Post-match effects (merged: momentumDelta, attrEffects, and baseline deltas)
  ...
  const aeMap = (det.attrEffects)    || (dbg.attrEffects)    || {}; // NEW

  (rec.names||[]).forEach(n=>{
    const w = rmap.get(n);
    const base = baseMap[n]?.values || null;

    // collect deltas in a single small map so we don't duplicate lines
    const deltas = {};

    // 1) engine-recorded momentum change
    if (typeof mdMap[n] === 'number' && mdMap[n] !== 0) {
      deltas.momentum = (deltas.momentum || 0) + mdMap[n];
    }

    // 2) engine-recorded per-attr effects (simulate.js -> explain.attrEffects)
    if (aeMap[n] && typeof aeMap[n] === 'object') {
      for (const [k,v] of Object.entries(aeMap[n])) {
    ...
    // 3) baseline diffs (authoritative, may override/augment)
    if (base && w){
      deltaKeys.forEach(k=>{
        const a = Number(w[k] ?? 0), b = Number(base[k] ?? a);
        const d = a - b;
        if (d !== 0) deltas[k] = (deltas[k] || 0) + d;
      });
    }

    const parts = Object.entries(deltas)
      .map(([k,d]) => `${labelFor(k)} ${d>0?'+':''}${d}`);
