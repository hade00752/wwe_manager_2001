...
import { pairKey, getRelLevel, REL } from './relationships.js';

function normBrand(b){
  const s = String(b||'').toUpperCase();
  if (s === 'GLOBAL') return 'GLOBAL';
  if (s === 'SD' || s.includes('SMACK')) return 'SD';
  if (s === 'RAW' || s.includes('RAW')) return 'RAW';
  return undefined;
}

function inboxForBrand(state, brand){
  ...
  return src
    ...
    .filter(m => !m.resolved)
    .filter(m => {
      const b = normBrand(m.brand);
      return b === 'GLOBAL' || b === brand || !b; // legacy/no-brand treated as on-brand
    });
}

function collectBookingSignals(state, brand){
  const msgs = inboxForBrand(state, brand);

  const mustUse = new Set();     // must appear somewhere
  const wantTag = new Set();     // must be in tag
  const avoidTop = new Set();    // avoid main event if possible

  for (const m of msgs){
    const title = String(m.title||'').toLowerCase();
    const from  = String(m.from || m.actor || '').trim();

    if (title.includes('left off the show') && from) mustUse.add(from);
    if (title.includes('wants more tag') && from) { mustUse.add(from); wantTag.add(from); }
    if (title.includes('walkout threat') && from) mustUse.add(from);

    // situations: use key as signal
    const sk = m?.situation?.key;
    if (sk === 'SAFETY_INCIDENT'){
      // usually mem.names[0] is the accused; be conservative
      const accused = m?.situation?.memory?.names?.[0];
      if (accused) avoidTop.add(accused);
    }
  }

  return { mustUse, wantTag, avoidTop };
}
...
export function aiBooking(state, brand){
  ...
  const signals = collectBookingSignals(state, brand);
  const FLOOR = {
    MAIN_AVG_STAR:     80,
    MAIN_WOMEN_STAR:   72,
    OPENER_AVG_WORK:   74,
  ...
  // Force priority appearances (complaints/situations) before “optimal” booking
  (function placeMustUse(){
    const pool = roster.filter(w=>!used.has(w.name));
    for (const name of signals.mustUse){
      const w = pool.find(x=>x.name===name);
      if (!w || used.has(w.name)) continue;

      // Put them in PreShow if empty, else Match if empty, else Promo1 if empty
      if (!segs.PreShow){
        // Find any same-gender opponent not used
        const opp = pool.find(x=>x.gender===w.gender && x.name!==w.name && !used.has(x.name));
        if (opp){ segs.PreShow = {type:"singles", a:w.name, b:opp.name}; used.add(w.name); used.add(opp.name); continue; }
      }
      if (!segs.Match){
        const opp = pool.find(x=>x.gender===w.gender && x.name!==w.name && !used.has(x.name));
        if (opp){ segs.Match = {type:"singles", a:w.name, b:opp.name}; used.add(w.name); used.add(opp.name); continue; }
      }
      if (!segs.Promo1){
        segs.Promo1 = {type:"promo", speaker:w.name};
        used.add(w.name);
      }
    }
  })();
  ...
  (()=> {
    ...
    // If avoidTop exists, temporarily filter those names out of main-event pool
    const mainPool = roster.filter(w=>!signals.avoidTop.has(w.name));
    const top = pickSinglesByStar(mainPool.length>=2 ? mainPool : roster, FLOOR.MAIN_AVG_STAR);
  ...
  (()=> {
    ...
    if(Array.isArray(tagHolder) && tagHolder.every(n=>get(n)) && Math.random()<CHANCE_TAG_DEFENSE){
      ...
      if(free(h1.name) && free(h2.name)){
        ...
        // If someone wants tag time, try to include them in the tag match.
        if (!segs.Tag && signals.wantTag.size){
          const target = [...signals.wantTag].find(n=>roster.some(w=>w.name===n && !used.has(n)));
          if (target){
            const T = roster.find(w=>w.name===target);
            const partner = roster.find(w=>w.gender===T.gender && w.name!==T.name && !used.has(w.name));
            const opp1 = roster.find(w=>w.gender===T.gender && ![T.name, partner?.name].includes(w.name) && !used.has(w.name));
            const opp2 = roster.find(w=>w.gender===T.gender && ![T.name, partner?.name, opp1?.name].includes(w.name) && !used.has(w.name));
            if (T && partner && opp1 && opp2){
              used.add(T.name); used.add(partner.name); used.add(opp1.name); used.add(opp2.name);
              segs.Tag = {type:"tag", teams:[[T.name, partner.name],[opp1.name, opp2.name]]};
            }
          }
        }
